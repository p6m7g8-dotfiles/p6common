...
#<
#
# Function: p6_date_fmt__date_bsd(input_date, input_fmt, out_fmt, offset, offset_fmt)
#
#  Args:
#	input_date - input date string
#	input_fmt - input format
...
#>
#/ Synopsis
#/    BSD date implementation: uses -j/-f/-v flags (macOS only).
######################################################################
p6_date_fmt__date_bsd() {
    local input_date="$1" # input date string
    local input_fmt="$2"  # input format
    local out_fmt="$3"    # output format
    local offset="$4"     # offset expression (BSD -v form)
...
p6_date_fmt__date_gnu() {
    ...
    if p6_string_blank_NOT "$date_spec"; then
        p6_date_fmt__cli "-d \"$date_spec\" +\"$out_fmt\""
    else
        p6_date_fmt__cli "+\"$out_fmt\""
    fi
}
